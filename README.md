...
- Writes `CodePushDeploymentKey` (and `CodePushServerURL` when you set `serverUrl`) to `Info.plist`.
...
- Writes `CodePushDeploymentKey` (and `CodePushServerUrl` when you set `serverUrl`) to `res/values/strings.xml`.
